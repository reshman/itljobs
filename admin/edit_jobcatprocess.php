...
//if(isset($_POST['submit'])){
   ...
   $industry     = $_POST['industry'];
    ...
    //Editing job_category table
    $sql = sprintf("UPDATE `job_categories` SET name='%s' WHERE id='%s' ",$title,$id); 
    $result = Db::query($sql);


    //Deleting existing records of job_category
    $query = sprintf("DELETE FROM industry_category WHERE category_id=%d", $id);
    $res = Db::query($query);
    ...
    //Adding new industry if not already present
     foreach ($industry as $each_industry) {
        if (!is_numeric($each_industry)) {
            ...
            $query = sprintf("INSERT INTO industry_category SET industry_id=%d,category_id=%d", $industry_id, $id);
