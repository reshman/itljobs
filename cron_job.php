...
if (mysql_num_rows($result) > 0) {
    while ($row = mysql_fetch_assoc($result)) {
        ...
        if ($count > 0) {
            ...
            $file = fopen("cron_job_log.txt", "a");
            fwrite($file,  date('Y-m-d h:i:s A') . ":Send Mail to " . $email. PHP_EOL);
            fclose($file);
    ...
    $file = fopen("cron_job_log.txt", "a");
    fwrite($file, date('Y-m-d h:i:s A') . ":No Mail to send for Cron Job " . PHP_EOL);
    fclose($file);
}