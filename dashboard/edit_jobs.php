...
<form role="form" name="frm" id="frm" method="POST" action="edit_jobsprocess.php" enctype="multipart/form-data">
    ...
    <div class="box-body">
            ...
            <select class="form-control" name="job_cat" id="job_cat">
                ...
                if (mysql_num_rows($res)) {
                    while ($row1 = mysql_fetch_array($res)) {
                        ?>
                        <option <?php if ($row['cid'] == $row1['id']) { ?> selected="" <?php } ?>
            ...
            <select class="form-control" name="title" id="title">
                ...
                if (mysql_num_rows($res)) {
                    while ($row1 = mysql_fetch_assoc($res)) {
                        ?>
                        <option <?php if ($row['title'] == $row1['industry_name']) { ?> selected="" <?php } ?>
                            value="<?php echo $row1['industry_name']; ?>"><?php echo $row1['industry_name']; ?></option>
                    <?php
                    }
                } ?>
            </select>
        ...
        <div class="form-group">
            ...
            <select type="text" class="form-control" id="company" placeholder="Company Name" name="company">
                ...
                while ($crow = mysql_fetch_assoc($result)) {
                    ?>
                    <option
                        value="<?= $crow['company_name'] ?>" <?php ($row['company_name'] == $crow['company_name']) ? 'selected' : ''; ?>><?= $crow['company_name'] ?></option>
                <?php } ?>
            </select>
        ...
        <div class="form-group">
            ...
            <textarea class="ckeditor" id="job_description"
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="create_date" placeholder="Create Date"
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="datepicker1" name="closing_date" placeholder="Closing Date"
                   data-format="yyyy-MM-dd" value="<?php echo $closing_date; ?>"/>
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="job_type">
                ...
                <option value="FULL TIME" <?php if ($row['job_type'] == 'FULL TIME') { ?> selected=""<?php } ?>>FULL
                    TIME
                </option>
                <option value="PART TIME" <?php if ($row['job_type'] == 'PART TIME') { ?> selected=""<?php } ?>>PART
                    TIME
                </option>
                <option value="TEMPORARY" <?php if ($row['job_type'] == 'TEMPORARY') { ?> selected=""<?php } ?>>
                    TEMPORARY
                </option>
                <option value="CONTRACT" <?php if ($row['job_type'] == 'CONTRACT') { ?> selected=""<?php } ?>>CONTRACT
                </option>
                <option value="INTERNSHIP" <?php if ($row['job_type'] == 'INTERNSHIP') { ?> selected=""<?php } ?>>
                    INTERNSHIP
                </option>
                <option value="FRESHER" <?php if ($row['job_type'] == 'FRESHER') { ?> selected=""<?php } ?>>FRESHER
                </option>
                <option value="WALKIN" <?php if ($row['job_type'] == 'WALKIN') { ?> selected=""<?php } ?>>WALK-IN
                </option>
            </select>
        ...
        $("#frm").validate({
            ...
            messages: {
                ...
                company: {
                    ...
                    lettersonly: "Please enter letters , '&' and '-' only"
                },
