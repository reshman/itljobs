                ...
                <div class="col-xs-12">
                    <div class="box">
                        ...
                        <div class="box-body" style="overflow-y: scroll;">
                            ...
                            ?>
                            <a href="export_job.php"><input type="button" class="btn btn-primary" name="submit"
                                                            value="Export"></a>
                            <table id="example2" class="table table-bordered table-hover">
                                ...
                                $query = sprintf("SELECT jc.id,j.id as jobid,j.job_listing,j.experience,j.job_location,j.created_date,j.closing_date,j.job_category_id,j.active,j.job_order,j.ref_id FROM jobs as j WHERE del_status='%s' AND closing_date>='%s' AND j.job_order!='%d' ORDER BY job_order", 0, $today_date, 0);
                                ...
                                while ($row = mysql_fetch_array($result)) {
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="viewmore_jobs.php?id=<?php echo $row['jobid']; ?>" target="_BLANK">view
                                                more</a></td>
                                        <td><input type="number" min="0" name="order" id="order" class="order"
                                                   value="<?php echo $row['job_order']; ?>"/>
                                        ...
                                        <td>
                                            <input <?php echo ($row['active'] == '1') ? 'checked' : ''; ?>
                                                rowid="<?php echo $row['jobid']; ?>" data-on="Active"
                                                data-off="Inactive" class="toggle-event" data-toggle="toggle"
                                                type="checkbox">
                                        </td>
                                        <td class=center><a href="edit_jobs.php?id=<?= $row['jobid'] ?>"
                                                            class="btn btn-warning "><i class="fa fa-edit"></i></a></td>
                                        <td class=center><a href="javascript:void(0)"
                                                            onclick="deleteConfirm('delete_jobs.php?delid=<?= $row['jobid'] ?>')"
                                ...
                                while ($row = mysql_fetch_array($res)) {
                                    ...
                                    if ($filrow['role_id'] == 1 || $filrow['role_id'] == 2) {
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo $row['job_location']; ?></td>
                                            <td><?php echo date('d/m/Y', strtotime($row['closing_date'])); ?></td>
                                            <td><a href="viewmore_jobs.php?id=<?php echo $row['jobid']; ?>"
                                                   target="_BLANK">view more</a></td>
                                            <td><input type="number" min="0" name="order" id="order" class="order"
                                                       value="<?php echo $row['job_order']; ?>"/>
                                                <a onclick="updatecheck(this)" class="btn btn-primary">update</a>
                                                <input type="hidden" name="id" id="id"
