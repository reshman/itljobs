  ...
  <input type="text" placeholder="COUNTRY" id="box2">
   <input type="submit" value="SEARCH JOBS" id="btn-search">
       ...
       <h1>UPCOMING INTERVIEWS</h1></div>
        <?php
            include 'db.php';
            $query = sprintf("SELECT js.id,js.job_listing,js.job_description,js.active,js.del_status,inv.title,inv.active,inv.del_status FROM jobs as js JOIN interviews as inv ON js.id=inv.title WHERE js.active=1 AND inv.active=1 AND js.del_status=0 AND inv.del_status=0");
       ...
       <div id="apply"><a href="itljobs-registration.php"><input type="submit" id="submit_contact" value="APPLY"></a></div>
       <div id="view"><input type="submit" id="submit_contact" value="VIEW MORE"></div>        
       </div>        
       </div>
        <?php
            }
