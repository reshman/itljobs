...
<!-- home-section
                  ================================================== -->
<?php
$skeyword = trim($_GET['q']);
$slocation = trim($_GET['l']);
$keyword = $_GET['q']!='' ? '%' . trim($_GET['q']) . '%' : '%';
$location = $_GET['l']!='' ? '%' . trim($_GET['l']) . '%' : '%';
        ...
        <div class="row">
            <div class="sea-box">
                <form method="GET" action="search-job.php" enctype="multipart/form-data">
                    <input type="text" value="<?php echo $skeyword; ?>" placeholder="JOB TITLE,KEYWORDS" id="box1"
                           name="q">
                    <input type="text" value="<?php echo $slocation; ?>" placeholder="COUNTRY" id="box2" name="l">
...
<section class="register-section">
    <div class="container">
        ...
        if ($location != '%' AND $keyword != '%') {
            $query = sprintf("SELECT j.id,j.job_listing, j.experience,j.ref_id, j.job_description, j.job_location, jc.name,j.closing_date,j.active  from jobs j LEFT JOIN  job_categories jc ON j.job_category_id = jc.id  WHERE j.job_location LIKE '%s' AND (jc.name LIKE '%s' OR j.job_listing LIKE '%s' OR j.key_array LIKE '%s' OR j.job_description LIKE '%s') AND j.closing_date>='%s' AND j.active='%s' AND j.del_status='%s' LIMIT %d,%d", $location, $keyword, $keyword, $keyword, $keyword, $date, '1', '0', $start, $num_rec_per_page);
        } else if ($location == '%' AND $keyword != '%') {
            $query = sprintf("SELECT j.id,j.job_listing, j.experience,j.ref_id, j.job_description, j.job_location, jc.name,j.closing_date,j.active  from jobs j LEFT JOIN  job_categories jc ON j.job_category_id = jc.id  WHERE  (jc.name LIKE '%s' OR j.job_listing LIKE '%s' OR j.key_array LIKE '%s' OR j.job_description LIKE '%s') AND j.closing_date>='%s' AND j.active='%s' AND j.del_status='%s' LIMIT %d,%d", $keyword, $keyword, $keyword, $keyword, $date, '1', '0', $start, $num_rec_per_page);
        } else if ($location != '%' AND $keyword == '%') {
            ...
            while ($row = mysql_fetch_array($result)) {
                ...
                <div class="jobs-box">
                    ...
                    <div class="col-md-12">
                        ...
                        <p><span
                                style="color:#6495ED">Experience : </span><?php echo ($row['experience'] == 0) ? $row['experience'] . ' year' : $row['experience'] . ' years'; ?>
                            ,
                            <span style="color:#6495ED">Location : </span><?php echo $row['job_location']; ?>,
                            <span
                                style="color:#6495ED">Closing date : </span><?php echo date("d/m/Y", strtotime($row['closing_date'])); ?>
                            <span style="color:#6495ED">Reference Id : </span><?php echo $row['ref_id']; ?></p>
                        <?php if (isset($_SESSION['log'])): ?>
                            <div id="apply"><a href="javascript:void(0)"
                                               onclick="apply(<?php echo $row['id'] ?>, this)"><input type="submit"
                                                                                                      value="<?php echo (in_array($row['id'], $jobsArray)) ? 'APPLIED' : 'APPLY' ?>"></a>
                            </div>
                            <?php if (!in_array($row['id'], $jobsArray)): ?>
                                <div id="view"><a href="javascript:void(0)"
                                                  onclick="view(<?php echo $row['id'] ?>, this)"><input type="submit"
                                                                                                        value="<?php echo (in_array($row['id'], $jobsSaveArray)) ? 'SAVED' : 'SAVE' ?>"></a>
                                </div>
                            <?php endif; ?>
                        <?php else: ?>
                            <div id="apply"><a href="javascript:void(0)" data-toggle="modal"
                                               data-target="#myModal"><input type="submit" value="APPLY"></a></div>
                            <div id="view"><a href="javascript:void(0)" data-toggle="modal"
                                              data-target="#myModal"><input type="submit" value="SAVE"></a></div>
                        <?php endif; ?>
                    </div>
                </div>
            <?php
        ...
        if (isset($total_pages) AND $total_pages > 1) {
            ?>
            <center>
                <ul class="pagination">
                    <li <?php if ($page == 1) { ?> class='disabled' <?php } ?>><a
                            href="search-job.php?q=<?= $keyword ?>&l=<?= $location ?>&p=1"><<</a></li>
                    <?php for ($i = 1; $i <= $total_pages; $i++) { ?>
                        <li <?php if ($page == $i) { ?> class='active' <?php } ?>><a
                                href="search-job.php?q=<?= $keyword ?>&l=<?= $location ?>&p=<?= $i ?>"><?= $i ?></a>
                        </li>
                    <?php } ?>
                    <li <?php if ($page == $total_pages) { ?> class='disabled' <?php } ?>><a
                            href="search-job.php?q=<?= $keyword ?>&l=<?= $location ?>&p=<?= $total_pages ?>">>></a></li>
                </ul>
            </center>
        <?php
        }
            ...
            while ($row = mysql_fetch_array($result)) {
                ...
                <div class="jobs-box">
                    ...
                    <div class="col-md-12">
                        ...
                        <?php if (isset($_SESSION['log'])): ?>
                            <div id="apply"><a href="javascript:void(0)"
                                               onclick="apply_interview(<?php echo $row['id'] ?>, this)"><input
                                        type="submit"
        ...
        if ($flag) {
            ?>
            <h3 style="text-align: center">Search returned Zero results</h3>
        <?php } ?>
    </div>
</section>
<!-- footer
